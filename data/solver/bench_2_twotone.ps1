# Benchmark 2: Two-Tone Connected (Jh 9h 4c) - full 3-street solve
# Usage: powershell -ExecutionPolicy Bypass -File bench_2_twotone.ps1

$ErrorActionPreference = "Continue"
$solverDir = Join-Path $PSScriptRoot "texassolver"
$solverExe = Join-Path $solverDir "console_solver.exe"
$inputFile = Join-Path $PSScriptRoot "inputs\bench_twotone_Jh9h4c.txt"
$outputFile = Join-Path $solverDir "bench_twotone_Jh9h4c.json"

$content = Get-Content $inputFile -Raw
$absOutput = ($outputFile -replace '\\', '/')
$content = $content -replace 'dump_result .*', "dump_result $absOutput"
$tempInput = Join-Path $solverDir "_bench_input_2.txt"
Set-Content -Path $tempInput -Value $content -NoNewline

Write-Host "============================================"
Write-Host "  BENCHMARK 2: Two-Tone (Jh 9h 4c)"
Write-Host "  Full flop + turn + river solve"
Write-Host "============================================"
Write-Host ""

$sw = [System.Diagnostics.Stopwatch]::StartNew()
$proc = Start-Process -FilePath $solverExe -ArgumentList "-i", "_bench_input_2.txt" -WorkingDirectory $solverDir -Wait -NoNewWindow -PassThru
$sw.Stop()
