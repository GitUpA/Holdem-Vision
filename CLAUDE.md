...
- **Complete preflop table**: `data/solver/complete_preflop_tables.json` (3380 cells)
  - Generated by `data/solver/generatePreflopTable.mjs` from validated GTO ranges
  - 5 archetypes × 6 positions × 169 hand classes with mixed frequencies
  - No limp in RFI/BvB, premiums never fold >5%, position-aware edge gradients
  - Replaces PokerBench preflop data (kept in `data/pokerbench/` for reference only)
  - Loaded via `convex/lib/gto/tables/preflopHandClassData.ts`
  - To regenerate: `node data/solver/generatePreflopTable.mjs`
